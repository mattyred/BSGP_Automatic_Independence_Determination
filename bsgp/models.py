    ...
    def _fit(self, X, Y, lik, Xtest, Ytest, Ystd, **kwargs):
        ...
        if not self.model:
            for i in range(self.ARGS.n_layers):
                ...
                kernel = BgpFullRBF(variance=0.1, randomized=False, d=output_dim) if self.ARGS.precise_kernel else BgpSE(output_dim, ARD=True, lengthscales=float(min(X.shape[1], output_dim))**0.5)
                kerns.append(kernel)
        ...
        try:
            ...
            self.posterior_samples_kerlogvar = [list(self.model.posterior_samples[i].values())[-1].tolist() for i in range(self.ARGS.num_posterior_samples)] # LRBF-MOD
